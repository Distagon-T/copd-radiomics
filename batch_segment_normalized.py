...
def find_normalized_nifti(patient_dir):
    """
    在患者文件夹中找到归一化后的 CT（<患者>_normalized.nii / .nii.gz）。
    每个患者由 normalize_ct_batch.py 生成一份 1x1x1mm 各向同性、25-HU 离散化的
    归一化 CT；正常情况下每个患者只有一份，取第一个即可。
    返回 (文件路径, None)；找不到返回 (None, None)。
    """
    nii_files = sorted(glob.glob(os.path.join(patient_dir, "*_normalized.nii*")))
    if not nii_files:
        return None, None
    return nii_files[0], None
...
def process_patient(patient_folder_path, patient_name, output_base_dir):
    ...
    # ① 找到归一化 NIfTI
    nifti_path, _ = find_normalized_nifti(patient_folder_path)
    if nifti_path is None:
        info["status"] = "failed"
        info["error"] = "该文件夹下未找到 *_normalized.nii(.gz) 文件（请先运行 normalize_ct_batch.py）"
        return False, info

    info["selected_nifti"] = os.path.basename(nifti_path)
    info["slice_count"] = inspect_nifti(nifti_path)["slices"]
    info["volume_info"] = inspect_nifti(nifti_path)

    # ② 输出目录：<患者名>_masks（与 batch_segment.py 保持一致）
    ...
    try:
        ...
        print(f"   ✅ {patient_name} 分割成功！输入: {os.path.basename(nifti_path)}，"
...
def batch_segment_normalized(input_base_dir, output_base_dir, patients=None):
    print("🚀 启动「归一化 CT (_normalized.nii)」批量分割流水线...")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="找到每个患者文件夹中的归一化 NIfTI (*_normalized.nii/.nii.gz) 并进行全器官分割")
    parser.add_argument("-i", "--input", default="nifti_input",
                        help="患者 NIfTI 文件夹所在目录（含 normalize_ct_batch.py 生成的 *_normalized.nii*）")
    parser.add_argument("-o", "--output", default="seg_results_normalized",
                        help="分割结果输出目录（默认 seg_results_normalized，勿与原始 CT 的 seg_results 混用）")
    parser.add_argument("--patients", default=None,
                        help="只处理指定患者（逗号分隔），默认全部")
    args = parser.parse_args()

    wanted = args.patients.split(",") if args.patients else None
    batch_segment_normalized(args.input, args.output, wanted)
