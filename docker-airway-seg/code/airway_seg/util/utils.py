...
# MONAI API compatibility: try to import commonly used transforms and
# provide simple fallbacks if the installed MONAI version does not
# expose them under these names.
try:
    ...
    from monai.transforms import KeepLargestConnectedComponent, ToNumpy, AsDiscrete, CastToType

    class AddChannel(object):
        """Fallback AddChannel: add a channel axis at position 0 for numpy arrays."""
        def __call__(self, x):
            import numpy as _np
            return _np.expand_dims(x, axis=0)

    class SqueezeDim(object):
        """Fallback SqueezeDim: remove single-dimensional entries from the shape."""
        def __call__(self, x):
            import numpy as _np
            return _np.squeeze(x)

    class ToTensor(object):
        """Fallback ToTensor: convert numpy array to torch tensor."""
