# docker-radiomics-seg — TotalSegmentator 分割 + PyRadiomics 特征提取 (离线 Docker)

面向 **医院 Ubuntu 22 + 双 4090 服务器** 的一站式部署包：
完成 **TotalSegmentator 分割 → PyRadiomics 特征提取 → 合并 CSV** 全流程。

## 目录结构

```
docker-radiomics-seg/
...
2. **PyRadiomics**：每个掩膜提取 `shape + firstorder` 特征（含 `preCrop` 加速）。
3. **四类 COPD 表型指标**：
...
```bash
# 1. 收集 TotalSegmentator 权重（需先在开发机跑过一次 TotalSegmentator）
python prepare_package.py

# 2. 构建镜像
docker build -t radiomics-seg:latest .

# （完全离线时先）
# python prepare_package.py --wheels   # 下载 wheel 到 wheelhouse/
```
...
## 已知注意点
- **本包为快速版**：lung_vessels 只算 firstorder（跳过最慢的 shape 特征），
  单患者约 2~3 分钟。若需要全部耗时特征（含 lung_vessels 完整 shape、肺叶
  LoG/纹理、心肌 Wavelet），请用 `docker-radiomics-full` 完整版包。
- 内存：单患者峰值约 2~4 GB（取决于 CT 尺寸），双卡并行建议 `--shm-size=16g`。
...
conda create -n radiomics-seg python=3.10 -y
conda activate radiomics-seg
...
mkdir -p ~/radiomics-env && tar -xzf radiomics-seg_env.tar.gz -C ~/radiomics-env
