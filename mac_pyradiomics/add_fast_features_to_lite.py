...
# -*- coding: utf-8 -*-
"""
给已有 Lite radiomics JSON 补上 Fast 独有的特征（Mac 版）
=========================================================
只加缺失键，不动已有值。复用 radiomics_extract._build_extractor，保证与
Mac Fast 版特征完全一致。
...
用法（Mac，建议 --workers 8）：
  ...
  python add_fast_features_to_lite.py -n /Volumes/.../2026-02-nifti -s /Volumes/.../2026-02-seg --workers 8
  python add_fast_features_to_lite.py -n /Volumes/.../2026-05-nifti -s /Volumes/.../2026-05-seg --workers 8
"""
import argparse
import glob
import json
...
from radiomics_extract import _build_extractor, _to_jsonable  # noqa: E402
...
def parse_args():
    p = argparse.ArgumentParser(description="给 Lite radiomics JSON 补 Fast 增量特征（Mac 版）")
    ...
    p.add_argument("--workers", type=int, default=8, help="并行进程数（Mac M 芯片建议 8，内存不足用 4）")
    return p.parse_args()


def resolve_ct_path(info_json, patient, nifti_dir):
    """优先 info json selected_nifti；否则 nifti_dir/<patient>/*.nii.gz 最大层数。"""
    ...
    try:
        ...
        for roi in LOBE_ROIS + [MYOCARDIUM_ROI]:
            ...
            try:
                ...
                ext = _build_extractor(roi)  # Fast 引擎：肺叶 LoG+纹理 / 心肌 Wavelet
