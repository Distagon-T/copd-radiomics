...
# -*- coding: utf-8 -*-
"""
PyRadiomics 特征提取核心（Mac 版）
==================================
pyradiomics 是纯 CPU 计算（基于 SimpleITK + numpy），**无 CUDA 依赖**，
可在 Apple Silicon (M 芯片) / Intel Mac 上原生运行。

特征策略（与 Windows compute_patient_radiomics_fast.py 完全一致，跨端列名统一）：
  - lung_vessels : 只算 firstorder（跳过最慢的 shape，改由 vessel_advanced_features
                    提供分形维度 / BV5-BV10 / 中心线迂曲度与分支密度）
  - 心肌          : 全特征 + Wavelet
  - 肺叶          : shape/firstorder/GLCM/GLRLM/GLSZM + LoG(sigma=[1.0])
  - 其余 ROI      : shape + firstorder
  - 全部含 preCrop 加速 + 掩膜级多进程并行（--workers）

输出: <患者>_radiomics.json（与 Windows/Docker 版同格式，可共用合并脚本）
"""
import os
import numpy as np
...
# 压制 pyradiomics 信息级日志（GLCM symmetrical / force2D 等刷屏），只保留错误
logging.getLogger('radiomics').setLevel(logging.ERROR)
for _lg in list(logging.root.manager.loggerDict):
    if _lg.startswith('radiomics'):
        logging.getLogger(_lg).setLevel(logging.ERROR)

# 允许 SimpleITK/ITK 滤波用满全部核心（CPU 并行）
...
def _build_extractor(roi_name, lite=False):
    """按 ROI 类型构建引擎：
      - lite=True（--lite）: 全部 ROI 只用 shape+firstorder（旧 Lite 策略，快）
      - 默认 Fast           : 与 Windows compute_patient_radiomics_fast.py 完全一致
        · lung_vessels: 只算 firstorder（跳过最慢的 shape，由 vessel_advanced_features 提供）
...
def _worker_pyradiomics(args):
    """单掩膜 pyRadiomics（供 Pool.map 使用；worker 内读取 CT+掩膜，Mac spawn 需顶层函数）。"""
    ct_path, mask_path, roi_name, lite = args
    try:
        # 关键修复：每个 worker 只用 1 个 ITK 线程，避免多 worker×全核滤波内存爆炸挂死
        sitk.ProcessObject.SetGlobalDefaultNumberOfThreads(1)
        ct = sitk.ReadImage(ct_path)
        mask = sitk.ReadImage(mask_path)
        ext = _build_extractor(roi_name, lite=lite)
...
def extract_patient_radiomics(ct_path, mask_dir, patient_name, workers=2, lite=False):
    """对一个患者做特征提取（掩膜级多进程并行；pyradiomics 纯 CPU，无 GPU 依赖）。
    lite=True 用旧 Lite 策略（全部 shape+firstorder），否则用 Fast 策略。"""
    ...
    jobs = [(ct_path, os.path.join(mask_dir, f), f[:-len('.nii.gz')], lite) for f in mask_files]
    ...
    for (ctp, mpath, name, _lite), roi_feats in zip(jobs, results):
