...
# -*- coding: utf-8 -*-
"""
纯 PyRadiomics 特征提取入口（Mac 版）
=====================================
不包含分割逻辑——只读取【已有的 <患者>_masks 分割结果】+ 原始 CT，
计算 537 维特征（16 掩膜 shape+firstorder + 四类 COPD 表型指标）。
...
用法：
  python run_radiomics.py --nifti-dir /path/CT --seg-dir /path/seg \
                          [--patients id1,id2] [--force]
"""
import argparse
import glob
import json
...
def parse_args():
    p = argparse.ArgumentParser(description="Mac 纯 PyRadiomics 特征提取")
...
def process_patient(meta, nifti_dir, seg_dir, force=False, workers=2):
    ...
    t0 = time.time()
    feats = extract_patient_radiomics(ct_path, mask_dir, patient, workers=workers)
...
def main():
    ...
    print(f"发现 {len(patients)} 个患者，开始纯 PyRadiomics 提取（纯 CPU；单患者超时 {args.timeout}s）...")
    results = []
    failed = []
