...
# -*- coding: utf-8 -*-
"""
run_bronch_hemoptysis_2026_05.py
================================
支扩队列：咯血高危型 vs 无咯血普通型 判别（Radiomics + AirQuant）。

 1) 队列：610 例纯支气管扩张（主诊断 J47：支气管扩张/支气管扩张症/细支气管扩张症）
 2) Label:
      Label=1（咯血高危型）: 主诊断含「伴咯血」（支气管扩张伴咯血 + 支气管扩张症伴咯血）127 例
      Label=0（无咯血普通型）: 主诊断仅「支气管扩张 / 支气管扩张症 / 合并感染」480 例
      （细支气管扩张症 3 例不属于两类，排除）
 3) 特征：pyRadiomics（重点心脏/血管：heart_myocardium 心肌拉长、PA/Ao、肺血管）+ MATLAB AirQuant
 4) 消融：radiomics | airquant | radiomics+airquant
 5) 输出图 + report_bronch_hemoptysis_2026_05.md/.html
"""
import argparse
import base64
import os
...
MD_OUT = os.path.join(SEG, "report_bronch_hemoptysis_2026_05.md")
HTML_OUT = os.path.join(SEG, "report_bronch_hemoptysis_2026_05.html")
SEED = 42
N_BOOT = 200
TOP_N = 15
LABEL_COL = "HEMO_Label"
HEMO_KW = "伴咯血"
BRONCH_PREFIX = ["支气管扩张", "支气管扩张症", "细支气管扩张症"]
PLAIN_DIAGS = {"支气管扩张", "支气管扩张症"}
...
def build_labels():
    ...
    info["_diag"] = diag
    br = info[diag.apply(is_bronch)].copy()
    d = br["_diag"]
    br["is_hemo"] = d.str.contains(HEMO_KW)
    br["is_plain"] = d.isin(PLAIN_DIAGS) | d.str.contains("合并感染")
    br["labeled"] = br["is_hemo"] | br["is_plain"]
    br["HEMO_Label"] = np.nan
    br.loc[br["is_hemo"], "HEMO_Label"] = 1
    br.loc[br["is_plain"], "HEMO_Label"] = 0

    out = br.rename(columns={"_pid": "patient_id", "_diag": "main_diagnosis"})
    out = out[["patient_id", "Patient_ID", "main_diagnosis",
               "HEMO_Label", "is_hemo", "is_plain", "labeled"]]
    out.to_csv(LAB_OUT, index=False, encoding="utf-8-sig")

    n_hemo = int(out["is_hemo"].sum())
    n_plain = int(out["is_plain"].sum())
    n_oth = int((~out["labeled"]).sum())
    print(f"[cohort] 纯支扩 {len(out)}（咯血 {n_hemo} / 无咯血 {n_plain} / 排除 {n_oth}）")
    print("[诊断分布]")
    for dd, c in out["main_diagnosis"].value_counts().items():
        print(f"  {dd}: {c}")
    return out
...
def run_ablation(df, feats, tag, label):
    ...
    uni.to_csv(os.path.join(SEG, f"fusion_hemo_{tag}_univariate_top.csv"),
    ...
    imp.to_csv(os.path.join(SEG, f"fusion_hemo_{tag}_lr_coefficients.csv"),
               index=False, encoding="utf-8-sig")
    res["uni"] = uni
    return res
...
def plot_roc(results, out_path):
    ...
    plt.title(f"Bronchiectasis: Hemoptysis vs Non (n={len(y)}, pos={int(y.sum())})")
...
def md_to_html(md_text, b64img):
    ...
    html = ["<h1>支扩队列：咯血高危型 vs 无咯血普通型（Radiomics + AirQuant）</h1>"]
    in_table = False
    for ln in lines[3:]:
        if ln.startswith("## "):
...
def write_report(results, cohort, diag_dist):
    L = []
    L.append("# 支扩队列：咯血高危型 vs 无咯血普通型 判别（Radiomics + AirQuant）\n")
    L.append(f"> 报告时间：2026-08-22　|　纯支扩队列 n={cohort['bronch']}（咯血 {cohort['hemo_all']} / 无咯血 {cohort['plain_all']}）　|　"
             f"建模样本 n={cohort['model_n']}（咯血 {cohort['pos']} / 无咯血 {cohort['neg']}）\n")
    L.append("## 1. 队列与标签\n")
    L.append("| 步骤 | 说明 |")
    L.append("|---|---|")
    L.append("| 队列 | 610 例纯支气管扩张（主诊断 ICD J47：支气管扩张/支气管扩张症/细支气管扩张症） |")
    L.append("| Label=1 | 咯血高危型：主诊断含「伴咯血」（支气管扩张伴咯血 + 支气管扩张症伴咯血） |")
    L.append("| Label=0 | 无咯血普通型：主诊断仅「支气管扩张/支气管扩张症/合并感染」 |")
    L.append("| 排除 | 细支气管扩张症（3 例）不属于两类 |")
    L.append("| 特征 | pyRadiomics（重点心脏/血管：heart_myocardium、PA/Ao、肺血管）+ MATLAB AirQuant |")
    L.append("")
    L.append("## 2. 诊断构成\n")
    L.append("| 主诊断 | 例数 |")
    L.append("|---|---|")
    for d, c in diag_dist.items():
        L.append(f"| {d} | {c} |")
    L.append("")
    ...
    L.append("")
    L.append("## 5. 心脏/血管（heart/vessel/PA/Ao）特征单变量\n")
    hv = full["uni"][full["uni"]["feature"].str.contains(
        "heart|vessel|aorta|pulmonary_artery|PA_Ao", case=False, regex=True)]
    L.append("| 特征 | AUC | Cohen's d | p(MWU) |")
    L.append("|---|---|---|---|")
    for _, row in hv.sort_values("auc_dev", ascending=False).head(25).iterrows():
        L.append(f"| {row['feature']} | {row['auc_univ']:.3f} | {row['cohens_d']:+.2f} | {row['p_mwu']:.2g} |")
    L.append("")
    L.append("## 6. AirQuant 气道特征单变量\n")
    ...
    L.append("## 7. 图表\n")
    for fn, cap in [
        ("fig_roc_bronch_hemoptysis_2026_05.png", "图 1. 三组消融 5 折 CV 平均 ROC"),
        ("fig_univariate_auc_bronch_hemoptysis_2026_05.png", "图 2. 单变量 AUC Top 20（红=正向，蓝=负向）"),
        ("fig_univariate_auc_hv_bronch_hemoptysis_2026_05.png", "图 3. 心脏/血管特征单变量 AUC Top 20"),
        ("fig_boxplot_top8_bronch_hemoptysis_2026_05.png", "图 4. Top 8 特征按 无咯血/咯血 箱线图"),
        ("fig_consistency_radiomics_bronch_hemoptysis_2026_05.png", "图 5. radiomics 一致性森林图"),
        ("fig_consistency_airquant_bronch_hemoptysis_2026_05.png", "图 6. AirQuant 一致性森林图"),
    ]:
        L.append(f"![{cap}](figs/{fn})\n")
        L.append(f"*{cap}*\n")
    L.append("## 8. 结论\n")
    best = max(results, key=lambda r: r["auc"])
    L.append(f"- 支扩队列中影像特征对「咯血型 vs 无咯血型」的最佳判别 AUC = **{best['auc']:.3f} ± {best['auc_std']:.3f}**（{best['tag']}）。")
    L.append("- 假说：咯血本质是肺动脉压升高/支气管动脉畸形扩张破裂，会在心脏与血管组学留下代偿信号；关注 heart_myocardium、PA/Ao、肺血管特征。")
    md = "\n".join(L) + "\n"
...
def main():
    ...
    labels = build_labels()
    diag_dist = labels["main_diagnosis"].value_counts().to_dict()
    ...
    print(f"[model] 样本 {n}, 咯血 {n_pos} / 无咯血 {n - n_pos}")
    ...
    plot_roc(results, os.path.join(FIGDIR, "fig_roc_bronch_hemoptysis_2026_05.png"))
    ...
    plt.title("Hemoptysis vs Non Top 20 by |AUC-0.5|")
    plt.tight_layout(); plt.savefig(os.path.join(FIGDIR, "fig_univariate_auc_bronch_hemoptysis_2026_05.png"), dpi=150); plt.close()

    hv = uni[uni["feature"].str.contains("heart|vessel|aorta|pulmonary_artery|PA_Ao", case=False, regex=True)]
    hv_top = hv.sort_values("auc_dev", ascending=False).head(20)
    if len(hv_top):
        colors = ["#d62728" if a >= 0.5 else "#1f77b4" for a in hv_top["auc_univ"]]
        plt.figure(figsize=(8, 0.42 * len(hv_top) + 1.5))
        plt.barh(range(len(hv_top)), hv_top["auc_univ"], color=colors)
        plt.axvline(0.5, color="k", ls="--", lw=1)
        plt.yticks(range(len(hv_top)), [shorten(f, 46) for f in hv_top["feature"]], fontsize=8)
        plt.gca().invert_yaxis(); plt.xlabel("Univariate AUC"); plt.xlim(0, 1)
        plt.title("Heart/Vessel/PA-Ao features Top 20")
        plt.tight_layout(); plt.savefig(os.path.join(FIGDIR, "fig_univariate_auc_hv_bronch_hemoptysis_2026_05.png"), dpi=150); plt.close()
    ...
    for ax, c in zip(axes, top8):
        ...
        ax.boxplot([x[y == 0].dropna(), x[y == 1].dropna()], tick_labels=["Non-hemo", "Hemoptysis"], widths=0.6)
        try:
    ...
    for ax in axes[len(top8):]:
        ax.axis("off")
    fig.suptitle("Hemoptysis Top 8 features by class", fontsize=12)
    plt.tight_layout(); plt.savefig(os.path.join(FIGDIR, "fig_boxplot_top8_bronch_hemoptysis_2026_05.png"), dpi=150); plt.close()
    ...
    aq_topc = analyze(feats_aq)
    forest_plot(rad_top, "Hemoptysis Consistency: radiomics features",
                os.path.join(FIGDIR, "fig_consistency_radiomics_bronch_hemoptysis_2026_05.png"))
    forest_plot(aq_topc, "Hemoptysis Consistency: AirQuant features",
                os.path.join(FIGDIR, "fig_consistency_airquant_bronch_hemoptysis_2026_05.png"))

    cohort = {"bronch": len(labels),
              "hemo_all": int(labels["is_hemo"].sum()),
              "plain_all": int(labels["is_plain"].sum()),
              "model_n": n, "pos": n_pos, "neg": n - n_pos}
    write_report(results, cohort, diag_dist)
    return 0
