...
# -*- coding: utf-8 -*-
"""
run_hemoptysis_topmodel_2026_05.py
==================================
支扩咯血：Top 显著特征精简模型 + Bootstrap 验证，并融合进 report_bronch_hemoptysis_2026_05。

 1) 读取 rad+aq 单变量 CSV，按 |AUC-0.5| 取 top5/top10/top20
 2) 手工临床集（心肌拉长 + 右房 + T/D + 壁厚 + PA/Ao + 气管能量）
...
from run_bronch_hemoptysis_2026_05 import SEG, FIGDIR, Args, LABEL_COL, build_labels
...
UNI_CSV = os.path.join(SEG, "fusion_hemo_rad+aq_univariate_top.csv")

# 手工临床集（假说驱动）：心肌拉长 + 右房 + T/D + 壁厚 + PA/Ao + 气管能量
CLINICAL_SET = [
    "heart_myocardium::original_shape_Sphericity",
    "heart_atrium_right::original_shape_MeshVolume",
    "aq_TD_fwhm_all",
    "aq_GenLe2_Wall_Thickness_mm_mean",
    "lung_trachea_bronchia::original_firstorder_TotalEnergy",
    "PA_Ao_Diameter_Ratio",
]
...
def plot_roc2(models, out_path, n, n_pos):
    ...
    plt.title(f"Hemoptysis Top-model ROC (n={n}, pos={n_pos})")
...
def main():
    ...
    feats_all = [c for c in feats_all if c != "COPD_BCOS"]
    ...
    plt.title(f"Hemoptysis: reduced-model AUC (errorbar=bootstrap 95%CI, n={n})")
    plt.legend(loc="upper left", fontsize=8)
    plt.tight_layout()
    plt.savefig(os.path.join(FIGDIR, "fig_hemoptysis_model_auc_2026_05.png"), dpi=150)
    plt.close()
    print("AUC 图 ->", os.path.join(FIGDIR, "fig_hemoptysis_model_auc_2026_05.png"))

    # 图2: top10 vs full ROC
    plot_roc2([(r["model"], r["Xv"], y) for r in rows if r["model"] in ("top10", "rad+aq(full)")],
              os.path.join(FIGDIR, "fig_roc_hemoptysis_top_2026_05.png"), n, n_pos)
    ...
    for c in top10:
        new_sec.append(f"- `{c}`")
    new_sec.append("")
    new_sec.append("**手工临床集（假说驱动，6 特征）**：心肌拉长(Sphericity) + 右房体积 + T/D + 管壁厚度 + 气管能量 + PA/Ao 比值。")
    new_sec.append("")
    new_sec.append("![图 7. 精简模型 5 折 CV AUC（误差条 = bootstrap 95%CI）](figs/fig_hemoptysis_model_auc_2026_05.png)\n")
    new_sec.append("*图 7. 精简模型 5 折 CV AUC（误差条 = bootstrap 95%CI）*\n")
    new_sec.append("![图 8. top10 vs 全特征 5 折 CV 平均 ROC](figs/fig_roc_hemoptysis_top_2026_05.png)\n")
    ...
    old_concl = "## 8. 结论"
    if old_concl in md:
        md = md.replace(old_concl, new_md + "## 9. 结论")
    else:
        md = md.rstrip() + "\n\n" + new_md
    ...
    concl_note = (f"\n- 精简模型验证：**{best_top['model']}** 保持最高判别力 "
                  ...
                  f"稳定性 {best_top['stability']:.0%})，证明咯血判别信号主要由少量显著特征承载，非高维过拟合。\n")
    md = md.replace("- 假说：咯血本质是肺动脉压升高/支气管动脉畸形扩张破裂",
                    concl_note + "- 假说：咯血本质是肺动脉压升高/支气管动脉畸形扩张破裂")
